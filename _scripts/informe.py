...
API570_INTERVALOS = {
    "clase 1": (
        ...
        "en un periodo no mayor a 5 años, como lo indica API 570, tabla 1, "
    ...
    "clase 2": (
        ...
        "años, como lo indica API 570, tabla 1, tomando en cuenta los "
        "mismos CML's de la medición de espesores de la presente inspección."
    ),
    "clase 3": (
        "Programar la próxima inspección visual y Medición de espesores {obj} "
        "en un periodo no mayor a 10 años, como lo indica API 570, tabla 1, "
        "tomando en cuenta los mismos CML's de la medición de espesores de "
        "la presente inspección."
    ),
    "clase 4": (
        ...
        "mayor a 5 años, como lo indica API 570, tabla 1, tomando en cuenta "
        "los mismos CML's de la medición de espesores de la presente "
        "inspección. (opcional por ser tubería clase 04)."
    ),
    "punto de inyeccion": (
        "Programar la próxima inspección visual y Medición de espesores {obj} "
        "en un periodo no mayor a 3 años, como lo indica API 570, tabla 1, "
        "tomando en cuenta los mismo CML's de la medición de espesores de la "
        "presente inspección."
    ),
}
...
def build_reco1_textos(grupo, lineas_por_clase):
    """`lineas_por_clase`: dict {clase_normalizada: [tags...]}. Sección 9.7b:
    si TODAS las líneas comparten la misma Clase, un solo párrafo con "del
    grupo de tuberías <GRUPO>"; si el grupo mezcla Clases, un párrafo POR
    Clase mencionando "de la línea <TAG>" / "de las líneas <TAG1> y <TAG2>"."""
    ...
    if len(clases) == 1:
        ...
        for clase in clases:
            tags = lineas_por_clase[clase]
            if len(tags) == 1:
                obj = f"de la línea {tags[0]}"
            else:
                obj = f"de las líneas {', '.join(tags[:-1])} y {tags[-1]}"
...
def _patch_reco1(document, grupo, lineas):
    ...
    for ln in lineas:
        ...
        if clase == "clase 1":
            pass
        lineas_por_clase.setdefault(clase, []).append(ln["tag"])
