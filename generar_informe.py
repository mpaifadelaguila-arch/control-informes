import os
import pandas as pd
from informe import generar_word_informe, insertar_foto_unidad
...
def ejecutar_proceso_grupo(grupo_buscado, ruta_maestro, ruta_base_lineas, ruta_plantilla_word, dir_salida="salida_informes", ruta_foto=None):
    ...
    # Remplazar la foto de la unidad en el Word si se proporcionó
    if ruta_foto and os.path.exists(ruta_foto):
        try:
            insertar_foto_unidad(ruta_word_salida, ruta_foto)
            print("[✔] Foto de la unidad insertada en el informe correctamente.")
        except Exception as ex_foto:
            print(f"[!] No se pudo insertar la foto en el Word: {ex_foto}")
