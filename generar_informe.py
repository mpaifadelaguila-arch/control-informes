    ...
    if checklist_path and os.path.exists(checklist_path):
        try:
            ...
            for hoja, datos in parsed.items():
                ...
                items_or = checklist_mod.items_observados_o_rechazados(datos)
                if reco and items_or:
                    # BUG CORREGIDO 18/09/2026 (grupo GT-023): antes se pegaba
                    # el MISMO texto combinado en TODAS las filas O/R de la
                    # hoja (si 'recomendacion' era un solo string). Ahora
                    # exige una lista con un texto por fila, en el mismo
                    # orden que items_observados_o_rechazados(datos) --
                    # cada fila recibe SOLO su propia recomendación, nunca la
                    # de otro ítem/fila. Si 'recomendacion' es un string
                    # suelto solo se acepta cuando hay una única fila O/R
                    # (no hay ambigüedad posible en ese caso).
                    if isinstance(reco, str):
                        recos = [reco] if len(items_or) == 1 else None
                        if recos is None:
                            resumen_avisos.append(
                                f"[checklist:{hoja}] 'recomendacion' es un solo texto "
                                f"pero hay {len(items_or)} filas observadas/rechazadas "
                                "-- se requiere una LISTA con un texto por fila (mismo "
                                "orden que las filas O/R) para no duplicar el mismo "
                                "texto combinado en todas ellas. Hoja omitida, revisar "
                                "el JSON de hallazgos/recomendaciones.")
                            continue
                    else:
                        recos = list(reco)
                        if len(recos) != len(items_or):
                            resumen_avisos.append(
                                f"[checklist:{hoja}] la lista de 'recomendacion' tiene "
                                f"{len(recos)} elementos pero hay {len(items_or)} filas "
                                "observadas/rechazadas -- no coinciden 1 a 1. Hoja "
                                "omitida, revisar el JSON de hallazgos/recomendaciones.")
                            continue
                    # Se reemplaza el comentario de cada ítem observado u
                    # rechazado por SU PROPIA recomendación redactada (sección 9.1).
                    col_updates = {}
                    for it, texto in zip(items_or, recos):
                        cell_ref = f"{_col_idx_to_letter(it['col_comentario'])}{it['fila']}"
                        col_updates[cell_ref] = texto
                    if col_updates:
